...
JavaScript
 ...
 w-halo marsoln Poisonloc ngtmuzi pramper wangbin2015 查看所有贡献者
重新介绍 JavaScript（JS 教程）
...
这里我们使用了两个变量。for 循环中间部分的赋值表达式同时也用来判断是否为真——如果为真，那么循环继续。因为 i 每次递增 1，这个数组的元素会被逐个传递给 item 变量。当遇到一个假值元素（如 undefined ）时，循环结束。

注意这个技巧只能在你知道数组中不含假值（例如对象或DOM节点的数组）时才可以使用。如果想要遍历可能包含 0 或空字符串的数组，应该使用 i, len 的写法。
...
分享： Twitter Facebook Google+
文档标签和贡献者
 标签：  JavaScript 教程 进阶
 此页面的贡献者： w-halo, marsoln, Poisonloc, ngtmuzi, pramper, wangbin2015, Ende93, machao, Tienyz, susutou, xlyimi, ziyunfei, arniu2006, Breezewish, stage37, teoli, xuxun, Joe_Zhao, xcffl, ethertank, Mgjbot, Physacco, Carrie zhxj, Laser
 最后编辑者: w-halo, Mar 15, 2016, 12:52:42 AM
另见
JavaScript
教程:
JavaScript 指南
